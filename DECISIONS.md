...
|---|-----------|---------|-----------|-------|
| D-080 | Sprint 29: Reproducible Foundation implementatie | Task-type templates + output contracts als hardcoded kwaliteitsgarantie | AI-output onvoorspelbaarheid minimaliseren via expliciete in/out contracten per taaktype | 2026-03-12 |
| D-081 | Remote agents: pad-mapping + repo-sync als structurele oplossing | _map_paths_for_remote() + _ensure_remote_repo() in spawner.py | WSL/Windows paden zijn fundamenteel incompatibel met Ubuntu remote. Automatische vertaling + git sync is de enige schaalbare oplossing. | 2026-03-12 |
| D-082 | Claude auth op remote servers: credentials sync via scp | Kopieer ~/.claude/.credentials.json naar remote host via scp. Verifieer met echo ping \| claude --print. Multi-user via ~/.oa/accounts.json. Geen OAuth, geen browser, nooit meer. | scp is deterministisch, headless, en werkt zonder browser of OAuth dance. OAuth flow op remote servers is onbetrouwbaar en vereist handmatige tussenkomst. | 2026-03-12 |
