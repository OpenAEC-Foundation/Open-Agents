 ...
 * Supports tool_calls in chat completions with multi-turn loop.
 */

/** Mistral function calling tool definition (OpenAI-compatible format) */
...
/** Map AgentTool names to Mistral function definitions */
function mapToolsToMistral(tools: AgentTool[]): MistralToolDef[] {
  const toolDefs: Record<AgentTool, MistralToolDef> = {
    Read: {
      type: "function",
      function: {
        name: "Read",
...
/** Simulate tool execution — placeholder for non-Claude runtimes.
 *  See OpenAI runtime for rationale. */
  ...
  async *execute(config: RuntimeExecutionConfig): AsyncIterable<AgentEvent> {
    ...
    try {
      ...
      const tools = config.agent.tools.length > 0
        ? mapToolsToMistral(config.agent.tools)
        : undefined;
