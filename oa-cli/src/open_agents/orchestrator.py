...
def _create_orchestrator_workspace(
    ...
    claude_md = (
        ...
        f"## WORKER MODEL\n"
        f"Standaard: `--model {worker_model}`\n"
        f"Slim routen: laat `--model` weg voor automatische selectie op basis van taaktype:\n"
        f"- Lange context / research / veel bronnen → mistral-nemo (128k)\n"
        f"- Eenvoudige taken / formatting / listing → mistral:7b\n"
        f"- Alles anders → mixtral:8x7b (sterkste)\n\n"
        f"## STAPPEN\n"
